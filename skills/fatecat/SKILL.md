...
用 FateCat 的现有仓库能力，稳定执行结构化命理分析、交付层启动与 skill bundle 导出；不在 skill 层重写业务算法。
...
- 这个 skill 不在源仓库内复制整份 `fatecat_runtime/`；独立运行时只能通过导出脚本物化。
...
- Known limits:
  - 当前阶段的脚本默认优先复用源仓库作为 runtime
  - 独立 runtime 需要显式执行导出脚本后才会物化
