...
用 FateCat 的现有仓库能力和嵌入式 runtime 镜像，稳定执行结构化命理分析、交付层启动与 skill bundle 导出；不在 skill 层重写业务算法。
...
- 这个 skill 不把根仓库当作唯一运行时；当前已经维护一份嵌入式 `fatecat_runtime/` 镜像，但它需要通过同步脚本刷新。
...
- Steps:
  1. 执行 `bash skills/fatecat/scripts/bootstrap.sh`
  2. 执行 `bash skills/fatecat/scripts/health.sh --mode pure --json`
  3. 执行 `bash skills/fatecat/scripts/pure-analysis.sh --input-json '...' --pretty`
...
- Steps:
  1. 准备 `assets/config/.env`
  2. 执行 `bash skills/fatecat/scripts/health.sh --mode delivery --json`
  3. 执行 `bash skills/fatecat/scripts/serve-api.sh`
- Expected output / acceptance:
  - FastAPI 进程启动成功
...
- Steps:
  1. 执行 `bash skills/fatecat/scripts/export-runtime.sh --output /tmp/fatecat-skill-bundle`
  2. 检查输出目录下是否存在 `skills/fatecat/scripts/fatecat_runtime/`
  3. 在导出目录内继续执行 `bash skills/fatecat/scripts/bootstrap.sh`
- Expected output / acceptance:
...
- `references/migration-plan.md`: 从源仓库到独立 bundle 的迁移路线
- `references/troubleshooting.md`: 常见失败模式

## Maintenance

- Sources: `README.md`、`AGENTS.md`、`pyproject.toml`、`modules/fate_core/src/fate_core/cli.py`、`modules/telegram/src/main.py`、`assets/deploy/bootstrap_agent.sh`
- Verification path:
  - 命令帮助：`.venv/bin/fatecat --help`
  - 纯分析：`.venv/bin/fatecat pure-analysis --help`
  - 健康检查：`.venv/bin/fatecat health --help`
  - 交付层：`.venv/bin/fatecat serve --help`
  - skill 结构校验：`/home/lenovo/.codex/skills/auto-skill/scripts/validate-skill.sh skills/fatecat --strict`
- Last updated: 2026-04-20
- Known limits:
  - 嵌入式 `fatecat_runtime/` 是镜像，不是新的源码真相源
  - 若根仓库源码变化，需重新执行 `sync-runtime.sh` 保持镜像新鲜
